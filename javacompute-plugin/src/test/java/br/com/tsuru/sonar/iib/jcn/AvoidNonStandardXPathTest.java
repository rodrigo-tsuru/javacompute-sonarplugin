...
public class AvoidNonStandardXPathTest {
...
//	    }
  ...
  public void detected() {
    ...
    AvoidNonStandardXPathCheck check = new AvoidNonStandardXPathCheck();

    SourceFile file = JavaAstScanner
      .scanSingleFile(new File("src/test/files/AvoidNonStandardXPathCheck.java"), new VisitorsBridge(check,CLASSPATH_JAR));
    	

    // Check the message raised by the check
    checkMessagesVerifier.verify(file.getCheckMessages())
      .next().atLine(26).withMessage("Do not use non-standard XPath!")
      .next().atLine(27)
      .next().atLine(29);
  }
}
